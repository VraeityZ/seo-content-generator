...
def render_extracted_data():
    ...
    with st.expander("View Complete Extracted Data", expanded=True):
        ...
        with st.expander("Add Custom Entities", expanded=False):
            ...
            # Text area for new entities (one per line)
            new_entities = st.text_area(
                "Enter custom entities (one per line)",
                height=100,
                help="Each line will be added as a separate entity. These will be prioritized in content generation."
            )
            ...
            with col1:
                ...
                if st.button("Add Custom Entities"):
                    if new_entities:
                        ...
                        entity_list = [e.strip() for e in new_entities.split('\n') if e.strip()]
